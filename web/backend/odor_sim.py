"""
Odor Navigation simulation: Zigzag chemotaxis toward food source.

Fly alternates left-right turns while walking forward, simulating
cast-and-surge odor tracking behavior (like real Drosophila in plumes).
"""
...
def _generate_odor():
    """Zigzag walk simulating chemotaxis. 5s total."""
    import mujoco
    ...
    print(f"[Odor] Generating {TOTAL_MS}ms...", flush=True)
    t0 = time.time()
    ...
    for step_i in range(n_steps):
        ...
        # Zigzag chemotaxis: sinusoidal turn bias + forward drive
        # Turn frequency increases as fly "approaches" source (simulated)
        progress = t_ms / TOTAL_MS  # 0→1

        # Cast frequency: starts slow (wide zigzag), gets fast (narrow)
        cast_freq = 0.8 + 2.0 * progress  # Hz
        # Turn amplitude: starts wide, narrows
        cast_amp = 4.0 * (1.0 - 0.6 * progress)
        # Forward speed: starts moderate, increases as "odor stronger"
        fwd_speed = 10.0 + 4.0 * progress

        turn_bias = cast_amp * np.sin(2 * np.pi * cast_freq * t_s)

        cpg.intrinsic_freqs[:3] = fwd_speed + turn_bias   # left legs
        cpg.intrinsic_freqs[3:] = fwd_speed - turn_bias   # right legs
        ...
        if progress < 0.2:
            ...
            cpg.intrinsic_freqs[:] = 3.0  # slow down near source
            phase = 'arrived'
    ...
    sim.close()
    print(f"[Odor] {len(frames)} frames in {time.time()-t0:.1f}s", flush=True)
    return geom_names, frames


CACHE = CACHE_DIR / 'odor_5.0s.json'
...
class OdorBridge:
    ...
    def _emit(self, data):
        ...
        except Exception as e: print(f"[Odor] emit error: {e}", flush=True)
    def _run_safe(self):
        try: self._run()
        except Exception as e:
            import traceback; print(f"[Odor] ERROR: {e}", flush=True); traceback.print_exc()
    ...
    def _run(self):
        if CACHE.exists():
            ...
            geom_names, frames = _generate_odor()
